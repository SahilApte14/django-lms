...
def student_course_recomendation(request):
    courses_data = pickle.load(open('student_management_app/templates/student_template/courses.pkl', 'rb'))
    similarity = pickle.load(open('student_management_app/templates/student_template/similarity.pkl', 'rb'))
    def recommend(course):
        indexValue = course_list.index(course)
        distances = sorted(list(enumerate(similarity[indexValue])), reverse=True, key=lambda x: x[1])
        recommended_course_names = []
        for i in distances[1:7]:
            course_name = courses_data.iloc[i[0]]['course_name']
            recommended_course_names.append(course_name)
        return recommended_course_names

    course_list = courses_data['course_name'].tolist()
    selected_course = 'Front-End Web Development with React'  # Default selected course
    ...
    context = {
        'courses_list': course_list,  # Changed variable name here
        'recommended_courses': recommended_course_names,
    }
    return render(request, "student_template/student_course_recomendation.html", context)

def student_askgpt(request):
    if request.method == 'POST':
        try:
            msg = request.POST.get('student_askgpt') 
            print(msg)
            result=""
            with open('input.txt', 'w') as file:
                file.write(msg)
            subprocess.run(["node", "student_management_app/script.js"])
            with open('output.txt', 'r') as file:
                result = file.read()
            context = {"student_askgpt": result}
            return render(request, "student_template/student_askgpt.html", context)
